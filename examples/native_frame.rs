//! A single example that exercises every feature of `iced-native-frame` on
...
//!
//! ```text
//! ICED_NATIVE_FRAME_DECORATIONS=system cargo run --example native_frame
//! ```
//!
//! It demonstrates:
//!
...
use iced_native_frame::{DecorationMode, FrameAction, NativeFrame, NativeFrameConfig};
...
fn main() -> iced::Result {
    let mode = match decoration_mode_from_args() {
        ...
        Err(message) => {
            eprintln!("{message}");
            eprintln!("usage: native_frame [--decorations custom|system]");
...
fn decoration_mode_from_args() -> Result<DecorationMode, String> {
    ...
    let mut raw = std::env::var("ICED_NATIVE_FRAME_DECORATIONS").ok();
...
impl App {
    ...
    fn update(&mut self, message: Message) -> Task<Message> {
        match message {
            ...
            Message::FrameInstalled(Err(error)) => {
                eprintln!("Native frame installation failed: {error}");
    ...
    fn body(&self) -> Element<'_, Message> {
        let heading = text("iced-native-frame").size(28);
